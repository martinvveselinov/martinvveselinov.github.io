    ...
    /*  for(i = 0; i < total_stands; ++i){
      ...
      for(i = 0; i < total; ++i){
        ...
        var statusval = Math.floor(Math.random() * 2);
        if(statusval == 0){
          bikes[i] = new google.maps.Marker({status: "problem", position: posi, type: typ, battery: battery, map: map, icon: '../img/problematic.png'});
        }
        else{
          bikes[i] = new google.maps.Marker({status: "ok", position: posi, type: typ, battery: battery, map: map, icon: '../img/current.png'});
        }
        bikes[i].addListener('click', (function(marker, i) {
           return function() {
            ...
            infowindow = new google.maps.InfoWindow({
              content: "Status: " + bikes[i].status + "<br>Distance from you: " + dist + " m.<br>Bike type: " + bikes[i].type + "<br>Remaining battery: " + bikes[i].battery + "%<br><button type=\"buttton\" onclick=\"unlockBike(\'" +this.position+ "\',\'start\');\">Start session</button>"
            }); 
